...
MzOTAClass::MzOTAClass(){}

void MzOTAClass::begin(MZOTA_WEBSERVER *server, const char * username, const char * password){
  _server = server;
  ...
  setAuth(username, password);
    ...
    if (!__isPicoW) {
      MZOTA_DEBUG_MSG("RP2040: Not a Pico W, skipping OTA setup\n");
      return;
    }
  #endif
    ...
    _server->on("/update", HTTP_GET, [&](AsyncWebServerRequest *request){
      if(_authenticate && !request->authenticate(_username.c_str(), _password.c_str())){
        ...
        //request->send(LittleFS, "/web/update.html", "text/html");
      #else
        AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", MZ_HTML, sizeof(MZ_HTML));
      #endif
    ...
    _server->on("/update", HTTP_GET, [&](){
      ...
      _server->send_P(200, "text/html", (const char*)MZ_HTML, sizeof(MZ_HTML));
    });
  #endif
    ...
    _server->on("/ota/start", HTTP_GET, [&](AsyncWebServerRequest *request) {
      ...
      if (request->hasParam("mode")) {
        ...
        if (argValue == "fs") {
          MZOTA_DEBUG_MSG("OTA Mode: Filesystem\n");
          mode = OTA_MODE_FILESYSTEM;
        } else {
          MZOTA_DEBUG_MSG("OTA Mode: Firmware\n");
      ...
      if (request->hasParam("hash")) {
        ...
        MZOTA_DEBUG_MSG(String("MD5: "+hash+"\n").c_str());
        if (!Update.setMD5(hash.c_str())) {
          MZOTA_DEBUG_MSG("ERROR: MD5 hash not valid\n");
      ...
      // Start update process
        ...
        if (!Update.begin(update_size, mode == OTA_MODE_FILESYSTEM ? U_FS : U_FLASH)) {
          MZOTA_DEBUG_MSG("Failed to start update process\n");
          // Save error to string
          StreamString str;
          Update.printError(str);
          _update_error_str = str.c_str();
          _update_error_str.concat("\n");
          MZOTA_DEBUG_MSG(_update_error_str.c_str());
        }
      #elif defined(ESP32)  
    ...
    _server->on("/ota/upload", HTTP_POST, [&](){
      ...
      if (upload.status == UPLOAD_FILE_START) {
        ...
        _current_progress_size = 0;
          ...
          if (Update.end(true)) {
              MZOTA_DEBUG_MSG(String("Update Success: "+String(upload.totalSize)+"\n").c_str());
          } else {
              MZOTA_DEBUG_MSG("[!] Update Failed\n");
        ...
        MZOTA_DEBUG_MSG(String("Update Failed Unexpectedly (likely broken connection): status="+String(upload.status)+"\n").c_str());
      }
    });
  #endif
}

void MzOTAClass::setAuth(const char * username, const char * password){
  _username = username;
  _password = password;
  _authenticate = _username.length() && _password.length();
}

void MzOTAClass::clearAuth(){
  _authenticate = false;
}

void MzOTAClass::setAutoReboot(bool enable){
  _auto_reboot = enable;
}

void MzOTAClass::loop() {
  // Check if 2 seconds have passed since _reboot_request_millis was set
  if (_reboot && millis() - _reboot_request_millis > 2000) {
    MZOTA_DEBUG_MSG("Rebooting...\n");
...
void MzOTAClass::onStart(std::function<void()> callable){
    preUpdateCallback = callable;
}

void MzOTAClass::onProgress(std::function<void(size_t current, size_t final)> callable){
    progressUpdateCallback= callable;
}

void MzOTAClass::onEnd(std::function<void(bool success)> callable){
