...
extern void newBricks();
extern void checkBallLost();
extern void setPositionPaddle();
extern void newBalltoPlayer();
extern void BalltoFloat();
extern void moveBall();
extern void checkBallHitByPaddle();
extern void checkBallOutOfBounds();
extern bool checkBlockCollision();
...
void runBricks(void *p) {
  game->gamecount[BRICKS]++;
  bricksInit();
  ...
  while(bricksRunning){    
    ...
    else
    {
      ...
      {
        ledDriver->setPixel(109+i,colorArray[YELLOW],0); // lösche Eckpixel
      }
    }
    ...
    game->aktscore = scorePlayer;
    if ( game->aktscore > game->highscore[BRICKS] ) game->highscore[BRICKS] = game->aktscore;
#ifdef DEBUG_GAME
